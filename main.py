#Author Blake cameron date 24/05/2022 Version 0.9
#This code starts all the functions and helps the other codes to run.
def question1():
    print("Hello welcome to a Quiz I made about te reo there are 15 questions about te reo")
    print("First of all what is your name")
    name = input()
    print("Hello " + name)
    print("okay lets begin")
    question = input("Is this the correct spelling for welcome nau mai ").lower()
    if question == "yes" or question == "Yes": 
      print("Correct good job")
    elif question == "no" or question == "No":
      print("wrong")
    else:
      print("please answer yes or no")


question1()


def question2():
#This code is the short version for the first one I also used this as the 'No' template
  question = input("Is this the correct spelling for english lgorgi ").lower()
  if question == "No" or question == "no":
    print("Correct good job this is the correct spelling Ingarihi")
  elif question == "Yes" or question == "yes":
      print("wrong")
  else: 
    print("please answer yes or no")


question2()


def question3():
#this is also other template i use this is the 'Yes' template
  question = input("Is this the correct spelling for danger mōrearea ").lower()
  if question == "Yes" or question == "yes":
        print("Correct good job")
  elif question == "no" or question == "No":
      print("wrong")
  else:
    print("please answer yes or no")


question3()


def question4():
  question = input("Is this the correct spelling for yellowstone kōwhaiwhai ").lower()
  if question == "Yes" or question == "yes":
      print("Correct good job")
  elif question == "no" or question == "No":
      print("wrong")
  else:
    print("please answer yes or no")


question4()


def question5():
  question = input("Is this the correct spelling for star wars Patango Whatu ").lower()
  if question == "No" or question == "no":
    print("Correct good job this is the correct spelling for Pakanga Whetu")
  elif question == "Yes" or question == "yes":
    print("wrong")
  else:
    print ("please answer yes or no")
question5()


def question6():
  question = input("Is this the correct spelling for libary whare pukapuka ").lower()
  if question == "Yes" or question == "yes":
    print("Correct good job")
  elif question == "no" or question == "No":
    print("wrong")
  else:
    print("please answer yes or no")


question6()


def question7():
  question = input("Is this the correct spelling for spider pūngāwerewere ").lower()
  if question == "Yes" or question == "yes":
    print("Correct good job")
  elif question == "no" or question == "No":
    print("wrong")
  else:
    print("please answer yes or no")


question7()


def question8():
  question = input("Is this the correct spelling for face kākāriki ").lower()
  if question == "No" or question == "no":
    print("Correct good job this is the correct spelling face kanohi")
  elif question == "Yes" or question == "yes":
    print("wrong")
  else:
    print("please answer yes or no")


question8()

def question9():
  question = input("Is this the correct spelling for family whanau ").lower()
  if question == "Yes" or question == "yes":
    print("Correct good job ")
  elif question == "no" or question == "No":
    print("wrong")
  else:
    print("please answer yes or no")


question9()

def question10():
  question = input("Is this the correct spelling for congratulations nga mihi ").lower()
  if question == "yes" or question == "Yes":
    print("Correct good job")
  elif question == "no" or question == "No ":
    print("wrong")
  else:
    print("please answer yes or no")


question10()



def question11():
  question = input("Is this the correct spelling for main matuō ").lower()
  if question == "No" or question == "no":
    print("Correct good job this is the correct spelling main matua")
  elif question == "Yes" or question == "yes":
    print("wrong")
  else:
    print("please answer yes or no")

question11()


def question12():
  question = input("Is this the correct spelling for earthquake rū whenua ").lower()
  if question == "yes" or question == "Yes":
    print("Correct good job")
  elif question == "no" or question == "No ":
    print("wrong")
  else:
    print("please answer yes or no")


question12()


def question13():
  question = input("Is this the correct spelling for hello kia ōra ").lower()
  if question == "No" or question == "no":
    print("Correct good job this is the correct spelling kia ora")
  elif question == "Yes" or question == "yes":
    print("wrong")
  else:
    print("please answer yes or no")

question13()


def question14():
  question = input("Is this the correct spelling for trade unions uniana hokohoko ").lower()
  if question == "Yes" or question == "yes":
    print("Correct good job")
  elif question == "No" or question == "no":
    print("wrong")
  else:
    print("please answer yes or no")

question14()



def question15():
  question = input("Is this the correct spelling for good job mahi pai").lower()
  if question == "Yes" or question == "yes":
    print("Correct good job")
  elif question == "No" or question == "no":
    print("wrong")
  else:
    print("please answer yes or no")

question15()